...
describe "NullDB with no schema pre-loaded" do
  ...
  [:nulldb, :nulldb_postgres].each do |adapter|
    context "under #{adapter}" do
  ...
  context 'under nulldb_postgres only' do
    it "should allow creating a table with hstore columns" do
      ActiveRecord::Base.establish_connection :adapter => :nulldb_postgres
      ActiveRecord::Schema.define do
        create_table(:employees) do |t|
          t.hstore :attributes, :null => false
        end
      end
    end

    it "should allow enabling extensions" do
      ActiveRecord::Base.establish_connection :adapter => :nulldb_postgres
      ActiveRecord::Schema.define do
  ...
  [:nulldb, :nulldb_postgres].each do |adapter|
    context "under #{adapter}" do
      ...
      it "should set the @config instance variable so plugins that assume its there can use it" do
        Employee.connection.instance_variable_get(:@config)[:adapter].should == adapter
      end
...
describe NullDB::RSpec::NullifiedDatabase do
  shared_examples_for 'nullified database' do
  ...
  context 'for a non-postgres db' do
    before(:all) do
      ActiveRecord::Base.configurations['test'] = {'adapter' => 'nulldb'}
    end

    it 'uses the nulldb adapter' do
      NullDB::RSpec::NullifiedDatabase.adapter.should eq(:nulldb)
    end

    include_examples 'nullified database'
  end

  context 'for a postgres db' do
    before(:all) do
      ActiveRecord::Base.configurations['test'] = {'adapter' => 'postgresql'}
    end

    it 'uses the nulldb_postgres adapter' do
      NullDB::RSpec::NullifiedDatabase.adapter.should eq(:nulldb_postgres)
    end
